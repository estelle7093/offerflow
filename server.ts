...
async function verifyToken(req: express.Request, res: express.Response, next: express.NextFunction) {
  ...
  if (!auth?.startsWith("Bearer ")) return res.status(401).json({ error: "未认证" });
  try {
    const supabaseUrl = process.env.VITE_SUPABASE_URL || "";
    const serviceKey = process.env.SUPABASE_SERVICE_ROLE_KEY || process.env.VITE_SUPABASE_ANON_KEY || "";
    const sb = createClient(supabaseUrl, serviceKey);
    const { data: { user } } = await sb.auth.getUser(auth.split(" ")[1]);
    if (!user) return res.status(401).json({ error: "认证失效" });
    next();
  } catch { return res.status(401).json({ error: "认证出错" }); }
}
